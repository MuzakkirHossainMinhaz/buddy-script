...
export class ReplyService {
     ...
     */
    async getRepliesByComment(commentUuid, limit, sort, cursor, currentUserId) {
        ...
        const repliesWithLike = await this.attachIsLikedByMe(replies, 'reply', currentUserId);
     ...
     */
    async createReply(commentUuid, userId, content, parentReplyUuid) {
     ...
     */
    async updateReply(replyUuid, userId, content) {
     ...
     * @throws {ForbiddenError} if the caller is not the author
     */
    async deleteReply(replyUuid, userId) {
        const reply = await visibilityService.getVisibleReplyByUuid(replyUuid, userId);
        if (reply.userId !== userId) {
